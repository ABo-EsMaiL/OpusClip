...
- [X] T020 [P] [US2] Create `src/opusclip/subtitle/__init__.py` and `src/opusclip/subtitle/base.py` — define `SubtitleRenderer` ABC with method: `render(words: list[WordTiming], clip_start: float, clip_end: float, config: PipelineConfig) -> Path`. Define `WordTiming` dataclass
- [X] T021 [P] [US2] Create `src/opusclip/rendering/__init__.py` and `src/opusclip/rendering/base.py` — define `VideoRenderer` ABC with method: `render_clip(context: PipelineContext, clip: ClipCandidate, subtitle_path: Path) -> RenderedClip`. Define `RenderedClip` dataclass with `path`, `thumbnail_path`, `duration`, `resolution` fields
- [X] T022 [P] [US2] Create `src/opusclip/metadata/__init__.py` and `src/opusclip/metadata/base.py` — define `MetadataGenerator` ABC with method: `generate(clip: ClipCandidate, transcript_excerpt: str, config: PipelineConfig) -> ClipMetadata`. Define `ClipMetadata` dataclass with `title`, `description`, `hashtags`, `category` fields
- [X] T023 [US2] Create `src/opusclip/pipeline.py` — implement `Pipeline` class that accepts all providers via constructor injection, orchestrates the full pipeline (input → transcription → clip selection → face detection → rendering → subtitle → metadata → output), and passes `PipelineContext` between stages. Addresses ARCH-001 (global state) and ARCH-003 (no provider abstraction)
...
- [X] T024 [US2] Create `src/opusclip/transcription/whisper_provider.py` — implement `WhisperProvider(TranscriptionProvider)` wrapping `faster-whisper` with `large-v3` model. Port transcription logic from Cell 4 (L325-376). Include VRAM cleanup via `torch.cuda.empty_cache()`
- [X] T025 [US2] Create `src/opusclip/transcription/word_repair.py` — port `fill_missing_words()` from Cell 6 (L648-703) as a standalone function. Remove dead commented-out code block (CODE-004 at L680-687). Remove unused `_BAD_CHARS` regex (CODE-005 at L572-591). Remove unused `subtitle_font()` function (CODE-006 at L638-640)
- [X] T026 [US2] Create `src/opusclip/clip_selection/llm_selector.py` — implement `LLMClipSelector(ClipSelector)` wrapping `openai` SDK with configurable `base_url` and `model`. Port clip selection logic from Cell 5 (L481-541) including the 3-attempt retry loop
- [X] T027 [US2] Create `src/opusclip/face_detection/mediapipe_detector.py` — implement `MediaPipeFaceDetector(FaceDetector)` using `mediapipe.tasks.vision.FaceLandmarker` with `output_face_blendshapes=True`. Map `jawOpen` blendshape to `mouth_open_score`. Convert normalized coordinates to pixel coordinates. Addresses AI-001 (100MB model for 2 points) and PERF-001 (CPU-bound dlib HOG)
- [X] T028 [P] [US2] Create `src/opusclip/face_detection/smart_director.py` — port `SmartDirector` class from Cell 6 (L870-921) as a standalone module. Accept `FaceResult` objects instead of raw dlib rectangles. Recalibrate `speaking_mar` threshold for MediaPipe blendshape scale (0.0-1.0)
- [X] T029 [US2] Create `src/opusclip/metadata/llm_metadata.py` — implement `LLMMetadataGenerator(MetadataGenerator)` wrapping `openai` SDK. Port `gen_meta()` from Cell 7 (L1217-1249). Add 3-attempt retry logic with exponential backoff. Mark failures explicitly with `{"_error": str(e)}` instead of empty dict. Addresses REL-004
...
- [X] T040 [US3] In `src/opusclip/rendering/ffmpeg_optimized_renderer.py` — merge the `subtitles=` filter into the raw-pipe FFmpeg step to eliminate the second full re-encode pass. Single-pass render implemented: `ass=` filter, audio input, and fades are all applied within the pipe command; `-shortest` trims output to audio duration. `BrokenPipeError` handling verifies exit code before treating closure as expected. Addresses PERF-002
- [X] T041 [US3] In `src/opusclip/config.py` — the `encoder` field already existed (default: `libx264`). In `src/opusclip/utils/ffmpeg_utils.py` — added `build_encoder_args()` and `check_encoder_available()`. Both renderers call `_resolve_encoder()` which falls back to `libx264` with a warning when the configured encoder is unavailable. `--encoder` CLI flag added. Addresses PERF-002 research recommendation
- [X] T042 [US3] In `src/opusclip/clip_selection/llm_selector.py` — `max_llm_chars` was already configurable via `PipelineConfig` (default: 28000). Added `warnings.warn()` when the transcript is truncated to less than 50% coverage with coverage percentage and total character count. Addresses scalability analysis Section 9
- [X] T042a [US3] Created `src/opusclip/metrics.py` — `PipelineMetrics` dataclass with per-stage timing (context manager), clip render durations, total duration, API call/retry/failure counters, and a human-readable `report()`. Integrated into pipeline: all 10 steps wrapped with `measure_stage()`, clip render times recorded, report printed at finish. Addresses PERF-003
